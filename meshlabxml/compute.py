...
def parse_vertex_quality_stats(ml_log, log=None, print_output=False):
    """Parse the ml_log file generated by the hausdorff_distance function.
    ...
    Returns:
        dict: dictionary with the following keys:
            min (float): minimum hausdorff distance
            max (float): maximum hausdorff distance
            mean (float): mean hausdorff distance
            median (float): root mean square distance

    """
    quality_stats = {"min": 0.0,
                          "max": 0.0,
                          "mean": 0.0,
                          "median": 0.0,
                          "stddev": 0.0,
    ...
    with open(ml_log) as fread:
        ...
        m = re.search(r"Min\D+(\d+\.*\d*)\D+(\d+\.*\d*)\D+(\d+\.*\d*)\D+(\d+\.*\d*)\D+(\d+\.*\d*)\D+(\d+\.*\d*)", result)
        quality_stats["min"] = float(m.group(1))
        quality_stats["max"] = float(m.group(2))
        quality_stats["mean"] = float(m.group(3))
        quality_stats["median"] = float(m.group(4))
        quality_stats["stddev"] = float(m.group(5))
        quality_stats["variance"] = float(m.group(6))
